...
export class CacheRunner {
  ...
  constructor(
    ...
    for (let i = lastIndex; i >= 0; i--) {
      const underlying: DataStore =
        i === lastIndex ? this.underlying : this.caches[i + 1];
  ...
  step() {
    ...
    if (this.lastSimulatedCycle + 1 >= this.instructions.length) {
      console.log(
        "[CacheRunner.step] already at end;",
        "lastSimulatedCycle=",
        this.lastSimulatedCycle,
        "instructions.length=",
        this.instructions.length,
  ...
  run() {
    ...
    while (this.lastSimulatedCycle + 1 < this.instructions.length) {
      this.step();
    }

    console.log(
      "[CacheRunner.run] finish",
      "lastSimulatedCycle=",
      this.lastSimulatedCycle,
      "instructions.length=",
      this.instructions.length,
